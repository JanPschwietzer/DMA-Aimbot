// vmmdll.h : header file to include in projects that use VmmHandle.dll / VmmHandle.so
// 
...
*    -userinteract = allow VmmHandle.dll to, on the console, query the user for
...
*              4 = forensic mode with static named sqlite database (VmmHandle.sqlite3).
*              Example -forensic 4
*
* -- argc
* -- argv
...
* general close of the VmmHandle dll.
* -- hVMM
* -- return
*/
EXPORTED_FUNCTION _Success_(return)
...
/*
* Log a message using the internal MemProcFS VmmHandle logging system. Log messages
...
*     is put alongside VmmHandle.dll. (On Linux the .dll files aren't necessary).
